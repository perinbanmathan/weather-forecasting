...
def train_ml_models():
    """
    Train three classifiers on the exported dataset.
    If dataset small, train on full data and skip test-split.
    Returns (trained_models_dict, label_encoder) or (None, msg) if insufficient data.
    """
    df = export_dataset()
    if df.empty:
    ...
    # Fit encoders on columns
    try:
        df["helmet_enc"] = le_helmet.fit_transform(df["helmet"].astype(str))
        df["signal_enc"] = le_signal.fit_transform(df["signal"].astype(str))
        df["stopped_enc"] = le_stopped.fit_transform(df["stopped"].astype(str))
        df["label_enc"] = le_label.fit_transform(df["label"].astype(str))
    ...
    # If dataset is too small to split, train on all data
    trained = {}
    try:
        if len(df) < 4:
            # train on full dataset
            dt = DecisionTreeClassifier()
            knn = KNeighborsClassifier(n_neighbors=1 if len(df) < 3 else 3)
            lr = LogisticRegression(max_iter=200)
            dt.fit(X, y)
            knn.fit(X, y)
            lr.fit(X, y)
            trained["DecisionTree"] = dt
            trained["KNN"] = knn
            trained["LogisticRegression"] = lr
            return trained, le_label
        else:
...
def ml_predict(trained, label_encoder, helmet, signal, stopped, speed, limit):
    """
    Predict using trained models.
    Input categorical values should be raw strings ('yes'/'no', 'green'/'red', 'yes'/'no').
    Returns dict { model_name: "Safe"/"Violation" }.
    """
    # We'll perform simple on-the-fly encoding consistent with the training encoders by mapping common values.
    # Note: Because label encoders for features are not returned, we follow the same mapping used in export: helmet yes/no -> encoded via LabelEncoder fit on existing values.
    # To be robust, encode using the same simple scheme: helmet yes->1 else 0, signal green->1 else 0, stopped yes->1 else 0
    try:
        helm_val = 1 if str(helmet).lower() == "yes" else 0
        sig_val = 1 if str(signal).lower() == "green" else 0
        stop_val = 1 if str(stopped).lower() == "yes" else 0
        X = [[helm_val, sig_val, stop_val, int(speed), int(limit)]]
    except Exception:
        X = [[0, 0, 0, int(speed or 0), int(limit or 0)]]

    predictions = {}
    for name, model in trained.items():
        try:
            pred_numeric = model.predict(X)[0]
            pred_label = label_encoder.inverse_transform([int(pred_numeric)])[0]
...
def classify_case():
    """
    Option A: return JSON result comparing Prolog reasoning and ML predictions.
    """
    init_rules_once()
    ...
    # Temporarily assert facts (replacing any existing facts for that driver/junction)
    if driver:
        prolog.query(f"retractall(wears_helmet({driver}, _))")
        prolog.query(f"retractall(stopped_at({driver}, _, _))")
        prolog.query(f"retractall(speed({driver}, _))")
    ...
    if driver:
        ...
        prolog.assertz(f"stopped_at({driver}, {junction or 'junction_0'}, {stopped})")
        prolog.assertz(f"speed({driver}, {speed})")
    if junction:
    ...
    if driver:
        ...
        # no driver provided: infer from facts (collect_all drivers)
        prolog_label = "Unknown (no driver provided)"

    # Train ML models (on the current dataset exported from Prolog)
    trained, encoder_or_msg = train_ml_models()
    if trained is None:
        # encoder_or_msg contains error message
        return jsonify({"error": encoder_or_msg}), 400

    # ML predictions for provided input
    ml_outputs = ml_predict(trained, encoder_or_msg, helmet, signal, stopped, speed, limit)
